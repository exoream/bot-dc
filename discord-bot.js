    ...
    if (command === "details") {
      ...
      if (!id) {
        return message.reply(
          "Silakan masukkan ID anime untuk melihat detail. Contoh: `!anime details kami-game-ueteiru-sub-indo`",
        )
      }
        ...
        const episodeList =
          filteredEpisodes.length > 0
            ? filteredEpisodes
                .map((episode, index) => {
                  return (
                    `**Episode ${filteredEpisodes.length - index}:** ${episode.episode_title}\n` +
